...
fn main() -> Result<()> {
    ...
    let result = (|| -> Result<()> {
        clear_screen()?;
        print_title("Welcome to Rosario's Enigma Encryption Tool")?;
        thread::sleep(Duration::from_secs(1));
            ...
            match choice.as_str() {
                "1" => {
                    match validate_password(&config) {
                        Ok(true) => print_centered("Password validated successfully!", Some(SUCCESS_COLOR))?,
                ...
                "4" => {
                    clear_screen()?;
                    print_title("Goodbye")?;
                    print_centered("Thank you for using Rosario's Enigma Encryption Tool.", None)?;
                    thread::sleep(Duration::from_secs(2));
                    break;
                }
                _ => {
